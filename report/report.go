...
//
////
//////Sectionwise Text Report Start.
////
//

type SectionWiseTextReportPage struct {
	Collegename     *string
	Sectionname     *string
	Sectionid       *string
	Textquestions   *[]question.TextQuestion
	Customfeedbacks *[]SectionWiseTextCustomFeedback
}

type SectionWiseTextCustomFeedback struct {
	Index     int
	Responses []string
}

func SectionWiseTextReportHandler(w http.ResponseWriter, r *http.Request) {
	log.Println("***SECTION WISE TEXT REPORT HANDLER***")
	log.Println("Serving client:", r.RemoteAddr)
	_, err := user.AuthenticateRequest(r)
	if err != nil { //user session not authentic. Redirect to login page.
		log.Println("User session is not authentic, displaying badpage.")
	...
	r.ParseForm()
	var myReportPage SectionWiseTextReportPage
	mySection, err := section.GetSection(template.HTMLEscapeString(r.Form.Get("sectionid")))
	if err != nil {
		displayBadPage(w, r, errors.New("Error finding given Section ID"))
		return
	...
	myReportPage.Sectionname = &mySection.Sectionname
	myReportPage.Sectionid = &mySection.Sectionid
	myReportPage.Textquestions, err = question.GetTextQuestions()
	log.Println("MyReportPage.Sectionname:", *myReportPage.Sectionname)

	if err != nil {
		displayBadPage(w, r, errors.New("Error finding text questions. Please check the database for valid text question entries"))
		return
	}

	myReportPage.Customfeedbacks, err = getSectionWiseTextCustomFeedbacks(mySection.Sectionid, myReportPage.Textquestions)
	if err != nil || myReportPage.Customfeedbacks == nil {
		log.Println("Problem getting custom feedbacks.")
		displayBadPage(w, r, errors.New("Error finding feedbacks for this section"))
		return
	...
	log.Println("Successfully got Customfeedbacks:", *myReportPage.Customfeedbacks, ". Executing Template.")
	filewriter, err := os.Create("sectionwisetext-" + mySection.Sectionid + ".csv")
	defer filewriter.Close()
	if err != nil {
		log.Println("Problem opening file for writing")
		displayBadPage(w, r, errors.New("Problem opening file for writing"))
	...
	templates.SectionWiseTextReportTemplate.Execute(filewriter, myReportPage)
	http.Redirect(w, r, "/report", http.StatusSeeOther)
	return
}

func getSectionWiseTextCustomFeedbacks(sectionId string, Textquestions *[]question.TextQuestion) (*[]SectionWiseTextCustomFeedback, error) {
	log.Println("**Getting Section Wise Text Custom Feedbacks**")
	myFeedbackSlice, err := feedback.GetFeedbacks(sectionId)
	if err != nil || myFeedbackSlice == nil {
		log.Println("Error finding Feedbacks:", err)
		return nil, err
	...
	var mySectionWiseTextCustomFeedbackSlice []SectionWiseTextCustomFeedback = make([]SectionWiseTextCustomFeedback, len(*myFeedbackSlice))
	for i, myFeedback := range *myFeedbackSlice {
		for _, textQuestionResponse := range myFeedback.Textquestionresponses {
			myMap[textQuestionResponse.Questionid] = html.UnescapeString(textQuestionResponse.Response)
		}
	...
	return &mySectionWiseTextCustomFeedbackSlice, nil
}
